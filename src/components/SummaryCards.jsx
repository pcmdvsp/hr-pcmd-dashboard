...
export default function SummaryCards({ employees }) {
  const cards = [{ key: 'total', label: 'Tổng quân số', value: employees.length, color: '#263d5a' }, ...Object.entries(STATUS).map(([key, info]) => ({ key, label: info.label, value: employees.filter(e => e.displayStatus === key).length, color: info.color }))]
