...
const COMPENSATORY_LEAVE_PDF_SYNC_LABEL = 'Synced from approved VSP PDF'
const isPdfSyncNotification = notification => notification.status === 'business_trip' && notification.content?.endsWith(PDF_SYNC_LABEL)
const isLeaveSyncNotification = notification => notification.status === 'leave' && ['Synced from VSP', COMPENSATORY_LEAVE_PDF_SYNC_LABEL].includes(notification.content)
...
export default function MeetingNotifications({ employeeId, onOpenMeetingInfo, readOnly = false }) {
  ...
  const load = useCallback(async () => {
    const [attendeeResult, viewResult, cancellationResult, statusResult, statusReadResult] = await Promise.all([
      ...
      supabase.from('status_update_notifications').select('id,employee_id,participant_ids,status,start_date,end_date,content,location,action,created_at').order('created_at', { ascending: false }).limit(100),
