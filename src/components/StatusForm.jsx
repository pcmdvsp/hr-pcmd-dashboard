...
const readDraft = (employeeId) => {
  try {
    return JSON.parse(sessionStorage.getItem(draftKey(employeeId)) || "null");
  } catch {
    return null;
  }
};
...
export default function StatusForm({
  ...
  // Token/profile refreshes replace the profile object after a tab regains
  // focus. Do not reinitialize a draft (and lose selected participants) unless
  // the actual employee changes.
  }, [employee.id, onClose, initialDate]);
  useEffect(() => {
    if (onClose || !draftInitialized || saved) return;
    sessionStorage.setItem(
      draftKey(employee.id),
      JSON.stringify({
        status,
        startDate,
        endDate,
  ...
  useEffect(() => {
    if (!onClose && saved) sessionStorage.removeItem(draftKey(employee.id));
  }, [employee.id, onClose, saved]);
  useEffect(() => {
    if (status !== "meeting" && !(status === "business_trip" && !onClose))
      return;
    Promise.all([
      supabase
        .from("profiles")
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return query
      ? employees.filter(
          (item) =>
            item.id !== employee.id &&
            `${item.full_name} ${item.employee_code}`
              .toLowerCase()
              .includes(query),
        )
      : [];
  ...
  const addDepartmentParticipants = (departmentId) => {
    ...
    const members = employees.filter(
      (item) =>
        item.department_id === departmentId && !unavailable.has(item.id),
    );
    setSelectedIds((ids) => [
      ...new Set([...ids, ...members.map((member) => member.id)]),
    ]);
  ...
  const submit = async (event) => {
    ...
    if ((status === "working" || status === "meeting") && !overtimeApproved) {
      ...
      const selectedWeekendDates = dates.filter((date) => {
        const dayType =
          calendarTypes.get(date) ||
          (new Date(`${date}T12:00:00`).getDay() % 6 === 0
            ? "weekend"
            : "working_day");
    ...
    if (status === "meeting") {
      ...
      if (status !== "working") {
        ...
        if (meetingsResult.error) {
          setSaving(false);
          return setError(meetingsResult.error.message);
        }
        const meetingIds = (meetingsResult.data || []).map(
          (meeting) => meeting.id,
        );
        const cancelOwnResult = await supabase
          .from("employee_meetings")
          .delete()
          .eq("organizer_id", employee.id)
          .in("date", dates);
        if (cancelOwnResult.error) {
          setSaving(false);
          return setError(cancelOwnResult.error.message);
        }
        if (meetingIds.length) {
          ...
          if (leaveResult.error) {
            setSaving(false);
            return setError(leaveResult.error.message);
          }
        }
      }
      ...
      const result = groupBusinessTrip
        ...
        : status === "working"
          ? await Promise.all([
              ...
              overtimeDates.length
                ? supabase.from("daily_status").upsert(
          ...
          : await supabase.from("daily_status").upsert(
              dates.map((date) => ({
                ...
                status,
                is_overtime: false,
                note: needsDetails ? null : note.trim() || null,
                content: needsDetails ? content.trim() : null,
                location: needsDetails ? location.trim() : null,
                start_time: null,
                end_time: null,
              })),
              { onConflict: "employee_id,date" },
  ...
  return (
    <form ref={formRef} className="status-form" onSubmit={submit}>
      <div className="form-title">
        <div>
          <p className="eyebrow">UPDATE STATUS</p>
          {onClose && <h2>{employee.full_name}</h2>}
        </div>
        {onClose && (
          <button
            type="button"
        ...
        <>
          ...
          <div className="employee-list" aria-label="Meeting organizer">
            ...
            >
              ...
              <span className="employee-code">
                {organizerUnavailableReason
                  ? `Unavailable: ${organizerUnavailableReason}`
                  : "Meeting organizer"}
              </span>
            </button>
          </div>
          {selectedDepartment && (
            <div
              ...
              aria-label={`${selectedDepartment.name} members`}
            >
                ...
                .map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`employee-badge is-editable ${selectedIds.includes(item.id) ? "is-selected" : ""}`}
                      style={
                        reason
                          ? { opacity: 0.55, cursor: "not-allowed" }
                          : undefined
      ...
      {status === "business_trip" && !onClose && (
        <>
          <label>
            ...
            <input
              value={tripParticipantQuery}
              onChange={(event) => {
                setTripParticipantQuery(event.target.value);
                markChanged();
              }}
              placeholder="Search by name or employee ID"
            />
          ...
          >
            ...
            >
              {employee.full_name}{" "}
              <span className="employee-code">Trip organizer</span>
            </button>
            {selectedTripEmployees
                ...
                >
                  ...
                  <span>
                    <input
                      ...
                      onChange={(event) => {
                        setLocation(
                          event.target.checked ? KNT_MEETING_ROOM : "",
                        );
                        markChanged();
                      }}
                    />{" "}
        ...
        status !== "working" && (
          <label>
            ...
            <textarea
              ...
              placeholder={notePlaceholders[status]}
              rows="3"
            />
          </label>
        )
      ...
      {confirmOvertime && (
        <div className="overtime-confirm-backdrop">
          <section
            className="overtime-confirm-dialog"
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="overtime-confirm-title"
          >
            <p className="eyebrow">OVERTIME CONFIRMATION</p>
            <h2 id="overtime-confirm-title">
              Please confirm your overtime working on selected weekend?
            </h2>
