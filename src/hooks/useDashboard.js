...
export function useDashboard(selectedDate = today()) {
  ...
  const load = useCallback(async () => {
    ...
    const [profileRes, statusRes, deptRes] = await Promise.all([
    ...
    if (profileRes.error || statusRes.error || deptRes.error) setError(profileRes.error?.message || statusRes.error?.message || deptRes.error?.message)
    ...
    const people = (profileRes.data || []).map(person => {
      ...
      return { ...person, daily, dailyPeriod: getPeriod(person.id, daily), displayStatus: daily?.status || 'working' }
    })
    ...
    people.forEach(person => {
      ...
      if (person.department_id && !departmentById.has(person.department_id)) departmentById.set(person.department_id, { id: person.department_id, name: 'Phòng ban chưa cấu hình', sort_order: Number.MAX_SAFE_INTEGER })
    })
    setEmployees(people)
  ...
  useEffect(() => { load() }, [load])
  return { employees, departments, loading, error, date, reload: load }
}
