...
function App() {
  ...
  const data = useDashboard(selectedDate, session?.user?.id);
  const [page, setPage] = useState("dashboard");
  useEffect(() => {
    const setEnglishRequiredMessage = (event) => {
      if (event.target.validity?.valueMissing)
        event.target.setCustomValidity("Please fill out this field.");
    };
    const clearMessage = (event) => event.target.setCustomValidity?.("");
    document.addEventListener("invalid", setEnglishRequiredMessage, true);
    document.addEventListener("input", clearMessage, true);
    return () => {
      document.removeEventListener("invalid", setEnglishRequiredMessage, true);
  ...
  if (session === undefined || profile === undefined)
    return <main className="auth-layout">Initializing...</main>;
    ...
    return (
      <main className="auth-layout">
        <div className="auth-card">
          ...
          <p>
            You are signed in, but this account does not have a profiles record.
  ...
  if (profile.must_change_password)
    return <ChangePasswordPage refreshProfile={refreshProfile} />;
  ...
  return page === "admin" && profile.role === "admin" ? (
    ...
    <Dashboard
      ...
      goMeeting={() => setPage("meeting")}
      goProduction={() =>
        window.open(
          "https://nymphetamineanh.github.io/psc-dashboard-v2/",
          "_blank",
          "noopener,noreferrer",
        )
      }
      ...
      onDateChange={(date) => navigate(`/?date=${date}`)}
    />
  );
}
