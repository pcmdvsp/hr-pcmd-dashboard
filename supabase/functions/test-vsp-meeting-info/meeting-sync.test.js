...
import { meetingSyncArguments, syncMeetings } from './meeting-sync.js';
...
test('maps VSP UTC timestamps to Vietnam meeting columns', () => {
  assert.deepEqual(meetingSyncArguments(sample), {
    p_external_source: 'vsp_eoffice', p_external_id: 'external-1', p_organizer_id: 'profile-1',
...
test('rejects meetings that cannot be represented safely', () => {
  ...
  assert.throws(() => meetingSyncArguments({ ...sample, endDate: '2026-08-17T18:00:00Z' }), /crosses/);
...
test('reports created, updated and failed records without leaking session data', async () => {
  let call = 0;
  const client = {
  ...
  assert.deepEqual(result.records, [{ externalId: 'external-1', meetingId: 'meeting-1', action: 'created', attendeeCount: 2 }]);
  assert.deepEqual(result.errors, [{ externalId: 'external-2', error: 'Database rejected meeting.' }]);
...
test('removes unavailable attendees before RPC and reports them separately', async () => {
  ...
  assert.deepEqual(result.skippedAttendees, [{
    externalId: 'external-1', employeeCode: '18351', name: 'Nguyễn Văn A', reason: 'leave',
  }]);
...
test('leaves an existing external meeting and its attendees unchanged', async () => {
  ...
  const result = await syncMeetings({
    from: (table) => {
      ...
      return resultQuery(table === 'employee_meetings'
        ? { data: [{ id: 'meeting-existing', external_id: 'external-1' }], error: null }
        : { data: [], error: null });
    },
    rpc: async () => { rpcCalled = true; },
  }, [sample]);
  ...
  assert.deepEqual(result.records, [{
    externalId: 'external-1', meetingId: 'meeting-existing', action: 'unchanged', attendeeCount: null,
  }]);
